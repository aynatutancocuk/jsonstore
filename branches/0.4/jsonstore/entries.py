...
class EntryManager(object):
    ...
    def create_entry(self, entry):
        ...
        """
        # Required elements.
        entry.setdefault('__id__', self._next_id())
        entry.setdefault('__updated__', time.strftime('%Y-%m-%dT%H:%M:%SZ', time.gmtime()))

        # Store entry.
        self.store[str(entry["__id__"])] = entry
        return entry
    ...
    def get_entries(self, size=None, offset=0):
        ...
        # Sort by time.
        entries.sort(key=operator.itemgetter('__updated__'), reverse=True)
    ...
    def update_entry(self, new_entry):
        ...
        entry = self.store[str(new_entry["__id__"])]
        entry.update(new_entry)

        # Update 'updated'.
        entry['__updated__'] = time.strftime('%Y-%m-%dT%H:%M:%SZ', time.gmtime())
